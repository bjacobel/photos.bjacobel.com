<img src="https://photos.bjacobel.bjacobel.com/logo.svg" alt="photos.bjacobel" width="200px">

[![Build Status](https://travis-ci.org/bjacobel/photos.bjacobel.svg?branch=master)](https://travis-ci.org/bjacobel/photos.bjacobel) [![codecov](https://codecov.io/gh/bjacobel/photos.bjacobel/branch/master/graph/badge.svg)](https://codecov.io/gh/bjacobel/photos.bjacobel) [![Dependency Status](https://gemnasium.com/badges/github.com/bjacobel/photos.bjacobel.svg)](https://gemnasium.com/github.com/bjacobel/photos.bjacobel) [![Greenkeeper badge](https://badges.greenkeeper.io/bjacobel/photos.bjacobel.svg)](https://greenkeeper.io/)

photos.bjacobel (React App Kit) is an opinionated skeleton to quickly set up a project with React, Redux, Webpack & friends.

### What's inside

photos.bjacobel includes and configures the following components to help you build a rock-solid, scalable app with best practices and zero configuration.
...
photos.bjacobel requires Node 6. In addition, to use the automatic AWS deployment features, you'll need both an AWS account and a Travis CI account.

Start a new Project with photos.bjacobel by creating an empty directory and installing `photos.bjacobel` into it.

(Note: This section's snippets use [`yarn`](https://yarnpkg.com) as a package manager. NPM works too. See below.)

    mkdir my-new-project
    cd my-new-project
    yarn add photos.bjacobel

Next, run the new `photos.bjacobel` command-line executable. It doesn't take any arguments, and will set up a project using the name of the folder you created as the project name.

    $(yarn bin)/photos.bjacobel

Some details about your newly created folder structure and dependencies will be printed while photos.bjacobel is setting up. After it's done, you can uninstall photos.bjacobel if you want:

    yarn remove photos.bjacobel
...
Note that while these instructions are given using Yarn, npm is also supported:
    ...
    npm install photos.bjacobel
    $(npm bin)/photos.bjacobel
    npm uninstall photos.bjacobel

### Deployment
photos.bjacobel includes a CloudFormation template that can create & configure all the AWS resources it needs. You'll want to create the CloudFormation stack before you push to your master branch for the first time. To do that:
...
### Thanks to
- all the developers of all the awesome code and systems photos.bjacobel is built on
